...
function renderReceitas() {
  ...
  tbody.innerHTML=items.map(r=>`<tr><td>${r.descricao}</td><td style="color:var(--accent);font-weight:700">${fmt(r.valor)}</td><td>${fmtDate(r.data)}</td><td><span class="chip chip-income">${r.categoria}</span></td><td><button class="action-btn edit" onclick="editReceita('${r.id}')"><iconify-icon icon="mdi:pencil-outline" width="17"></iconify-icon></button><button class="action-btn del" onclick="confirmDelete('receita','${r.id}')"><iconify-icon icon="mdi:trash-can-outline" width="17"></iconify-icon></button></td></tr>`).join('');
}
...
function renderDespesas() {
  ...
  tbody.innerHTML=items.map(d=>`<tr><td>${d.descricao}</td><td style="color:var(--danger);font-weight:700">${fmt(d.valor)}</td><td>${fmtDate(d.data)}</td><td><span class="chip chip-expense">${d.categoria}</span></td><td><span class="chip chip-${d.status}">${d.status==='pago'?'Pago':'Pendente'}</span></td><td>${d.status==='pendente'?`<button class="action-btn pay" title="Marcar pago" onclick="marcarPago('${d.id}')"><iconify-icon icon="mdi:check" width="17"></iconify-icon></button>`:''}<button class="action-btn edit" onclick="editDespesa('${d.id}')"><iconify-icon icon="mdi:pencil-outline" width="17"></iconify-icon></button><button class="action-btn del" onclick="confirmDelete('despesa','${d.id}')"><iconify-icon icon="mdi:trash-can-outline" width="17"></iconify-icon></button></td></tr>`).join('');
}
